# `BloombergLP::bdlt::Time::setMinute(int)` - Assumed

```x86asm
000000000046ba50 <BloombergLP::bdlt::Time::setMinute(int)>:
0000000000000000: 01	pushq	%rbp
0000000000000001: 02	pushq	%r14
0000000000000003: 01	pushq	%rbx
0000000000000004: 04	subq	$48, %rsp
0000000000000008: 02	movl	%esi, %ebp
000000000000000a: 03	movq	%rdi, %rbx
000000000000000d: 10	movabsq	$274877906944, %r14
0000000000000017: 03	movq	(%rdi), %rcx
000000000000001a: 03	cmpq	%r14, %rcx
000000000000001d: 06	jl	0x46bb10 <BloombergLP::bdlt::Time::setMinute(int)+0xc0>
0000000000000023: 10	movabsq	$-274877906945, %rax
000000000000002d: 03	andq	%rax, %rcx
0000000000000030: 10	movabsq	$86399999999, %rax
000000000000003a: 03	cmpq	%rax, %rcx
000000000000003d: 02	jg	0x46bafb <BloombergLP::bdlt::Time::setMinute(int)+0xab>
000000000000003f: 10	movabsq	$-8130577079664715991, %rdx
0000000000000049: 03	movq	%rcx, %rax
000000000000004c: 03	imulq	%rdx
000000000000004f: 03	addq	%rcx, %rdx
0000000000000052: 03	movq	%rdx, %rax
0000000000000055: 04	shrq	$63, %rax
0000000000000059: 04	sarq	$25, %rdx
000000000000005d: 03	addq	%rax, %rdx
0000000000000060: 07	imulq	$60000000, %rdx, %rsi
0000000000000067: 10	movabsq	$-7442832613395060283, %rdx
0000000000000071: 03	movq	%rcx, %rax
0000000000000074: 03	imulq	%rdx
0000000000000077: 03	movq	%rcx, %rdi
000000000000007a: 03	subq	%rsi, %rdi
000000000000007d: 03	addq	%rcx, %rdx
0000000000000080: 03	movq	%rdx, %rax
0000000000000083: 04	shrq	$63, %rax
0000000000000087: 04	sarq	$31, %rdx
000000000000008b: 03	addq	%rax, %rdx
000000000000008e: 05	movl	$3600000000, %eax
0000000000000093: 04	imulq	%rdx, %rax
0000000000000097: 06	imull	$60000000, %ebp, %edx
000000000000009d: 03	addq	%rcx, %rdx
00000000000000a0: 03	subq	%rcx, %rax
00000000000000a3: 03	addq	%rdx, %rax
00000000000000a6: 03	addq	%rdi, %rax
00000000000000a9: 02	jmp	0x46bb01 <BloombergLP::bdlt::Time::setMinute(int)+0xb1>
00000000000000ab: 06	imull	$60000000, %ebp, %eax
00000000000000b1: 03	orq	%r14, %rax
00000000000000b4: 03	movq	%rax, (%rbx)
00000000000000b7: 04	addq	$48, %rsp
00000000000000bb: 01	popq	%rbx
00000000000000bc: 02	popq	%r14
00000000000000be: 01	popq	%rbp
00000000000000bf: 01	retq	
00000000000000c0: 05	movl	$7304160, %edi
00000000000000c5: 05	callq	0x46f120 <BloombergLP::bsls::Review::updateCount(BloombergLP::bsls::Atomic_TypeTraits<BloombergLP::bsls::AtomicOperations_ALL_ALL_ClangIntrinsics>::Int*)>
00000000000000ca: 09	movq	$5031888, 8(%rsp)
00000000000000d3: 09	movq	$5031938, 16(%rsp)
00000000000000dc: 08	movl	$203, 24(%rsp)
00000000000000e4: 09	movq	$5035034, 32(%rsp)
00000000000000ed: 04	movl	%eax, 40(%rsp)
00000000000000f1: 05	leaq	8(%rsp), %rdi
00000000000000f6: 05	callq	0x46f140 <BloombergLP::bsls::Review::invokeHandler(BloombergLP::bsls::ReviewViolation const&)>
00000000000000fb: 07	imulq	$1000, (%rbx), %rcx
0000000000000102: 05	jmp	0x46ba80 <BloombergLP::bdlt::Time::setMinute(int)+0x30>
0000000000000107: 09	nopw	(%rax,%rax)
```
